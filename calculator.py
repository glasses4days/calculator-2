...
"""New calculator using reduce()"""

while True:
    try: 
        """Checks if [n] are integers"""
        input_number = raw_input().lower()
        tokens = input_number.split(" ")
        user_numbers = tokens[1:]
        for i in user_numbers: #use range maybe ==> range(tokens)[1:]
             user_numbers[i] = float(user_numbers[i]) 
            print user_numbers
    #     num2 = float(tokens[2])
        if tokens[0] == "q":
            """ if [0] is q, then it will quit the loop"""
            ...
            if tokens[0] == "+":
                """Checks if [0] is +, then it calls add function"""
                print add(user_numbers[0], user_numbers[2], user_numbers[3])
    #         elif tokens[0] == "-":
    #             """Checks if [0] is -, then it calls subtract function"""
    #             print subtract(num1, num2)
    #         elif tokens[0] == "*":
